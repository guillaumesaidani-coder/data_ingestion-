...
def _bronze_row(
    machine_id, timestamp, temp, pressure=200.0, voltage=227.0, rotation=1590.0, pieces=10
):
    return {
...
def test_outlier_is_replaced_by_linear_interpolation():
    ...
    rows = [
        _bronze_row("MACH-01", f"2025-06-02 0{h}:00:00", temp)
...
@pytest.mark.requires_local_infra  # telemetry.csv est piloté par DVC, absent d'un checkout CI brut
