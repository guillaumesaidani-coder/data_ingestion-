...
def _batches(*, done_at=(), running_at=(), failed_at=()):
    rows = (
        [{"started_at": t, "status": "done"} for t in done_at]
        + [{"started_at": t, "status": "running"} for t in running_at]
        + [{"started_at": t, "status": "failed"} for t in failed_at]
    )
    return pd.DataFrame(rows, columns=["started_at", "status"])


def test_new_terrain_batch_gate_fails_when_no_batch_since_cutoff():
    batches = _batches(done_at=["2026-05-01T00:00:00+00:00"])
    ok, detail = check_new_terrain_batch(batches, pd.Timestamp("2026-06-01", tz="UTC"))
    assert not ok
    assert "aucun nouveau lot" in detail


def test_new_terrain_batch_gate_passes_on_fresh_done_batch():
    batches = _batches(done_at=["2026-05-01T00:00:00+00:00", "2026-06-15T00:00:00+00:00"])
    ok, detail = check_new_terrain_batch(batches, pd.Timestamp("2026-06-01", tz="UTC"))
    assert ok
    assert "1 lot" in detail


def test_new_terrain_batch_gate_ignores_running_batches():
    # Un batch en cours (jamais clos) ne doit pas suffire a ouvrir ce feu --
    # seul un batch 'done' prouve que le lot a bien traverse jusqu'a Gold.
    batches = _batches(running_at=["2026-06-15T00:00:00+00:00"])
    ok, _ = check_new_terrain_batch(batches, pd.Timestamp("2026-06-01", tz="UTC"))
    assert not ok


def test_new_terrain_batch_gate_handles_empty_history():
    batches = _batches()
    ok, detail = check_new_terrain_batch(batches, pd.Timestamp("2026-06-01", tz="UTC"))
    assert not ok
    assert "aucun nouveau lot" in detail
