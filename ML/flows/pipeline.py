"""Module 29 : flow Prefect orchestrant l'ETL Gold, l'entraînement (avec
reprise), le scoring des dernières mesures par machine, et le stockage
idempotent des prédictions.
...
- ensure-model saute l'entraînement si un modèle existe déjà sur disque
  (`retrain=False`, par défaut).
- store-predictions upsert par (machine_id, window_start) : un 2e passage
  laisse le même nombre de lignes, jamais le double.

Usage : uv run --frozen python flows/pipeline.py
...
DEFAULT_PREDICTIONS_DB = Path("artifacts/predictions.db")
...
def store_predictions(predictions: pd.DataFrame, db_path: Path) -> int:
    logger = get_run_logger()
    rows = [
        {
    ...
    rows_in_db = upsert_predictions(db_path, rows)
    logger.info(
        "%d prédictions upsertées -> %s (%d lignes en base)", len(rows), db_path, rows_in_db
    )
    return rows_in_db
...
def indusense_pipeline(
    ...
    predictions_db: Path = DEFAULT_PREDICTIONS_DB,
    retrain: bool = False,
) -> dict:
    ...
    rows_in_db = store_predictions(predictions, predictions_db)

    return {
        "rows_scored": len(predictions),
        "rows_in_db": rows_in_db,
        "db_path": str(predictions_db),
    }
