...
| TC-HITL-04a | Les 3 statuts de reclassification Streamlit sont couverts | `pytest tests/test_streamlit_review.py -v` | 4 passed (paramétré) — `FAUSSE_ALERTE`, `CAPTEUR_DEFAILLANT`, `MAINTENANCE_PREVENTIVE` | `tests/test_streamlit_review.py` |
...
| TC-HITL-06a | Règle d'or testée sur ses 3 branches, y compris cas limites | `pytest tests/test_arbitration.py -v` | 5 passed — `ACCEPTATION_DIRECTE`, `ACCEPTATION_SOUS_DEROGATION`, `REJET_DU_MODELE_N` couverts, dont le cas réel gains=9/régressions=7 et la limite exacte du ratio 3× | `tests/test_arbitration.py` |
...
| TC-HITL-08a | Les 4 feux Prefect testés isolément, vert et rouge | `pytest tests/test_retrain_flow.py -v` | 7 passed — chaque feu couvert indépendamment, dont l'injection d'une valeur physique absurde (9999 °C) sur le feu qualité | `tests/test_retrain_flow.py` |
...
| TC-HITL-11a | Mode fantôme testé unitairement (confirmation, sauvegarde, historique) | `pytest tests/test_shadow.py -v` | 9 passed | `tests/test_shadow.py` |
...
| Boucle HITL | 15 | ✅ (chaîne honnête : `REJET`/`PAS_ELIGIBLE` sont les résultats corrects sur ces données, pas des échecs) |
| **Total** | **52** | — |
...
## Limites connues — non testées, par construction

Deux écarts entre `feuille_de_route_mlops_indusense.md` et le code actuel ne sont
**pas** couverts par ce cahier, volontairement : ce sont des décisions produit non
tranchées, pas des oublis de test.

- **Mode fantôme statique, pas vivant.** La feuille de route (§7) décrit une
  observation silencieuse sur 7 à 14 jours de trafic réel avant bascule.
  `run_shadow_mode.py` (`src/indusense/shadow.py`) rejoue en réalité un holdout
  **statique** du Gold Dataset (fenêtre déjà connue, `test_df` au-delà d'une date de
  coupure) et rend son verdict instantanément — TC-HITL-10/11 le prouvent correctement,
  mais ne prouvent pas la capacité « observer en conditions réelles pendant N jours »
  décrite dans la feuille de route, qui n'existe pas dans le code.
- **Pas de registre MLflow.** La feuille de route (§8) recommande MLflow pour
  l'historique de fabrication de chaque version. `get_model_version()` utilise une
  simple empreinte du fichier `.joblib`, pas un registre versionné consultable.

Tant que ces deux capacités ne sont pas construites, il n'y a rien à tester dessus.
