...
def compute_rolling_zscore(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.copy()
    df["temp_zscore_24h"] = (df["temperature_c"] - df["temp_mean_24h"]) / df[
        "temp_std_24h"
    ].replace(0, float("nan"))
    df["pressure_zscore_24h"] = (df["pressure_bar"] - df["pressure_mean_24h"]) / df[
        "pressure_std_24h"
    ].replace(0, float("nan"))
    return df
...
def compute_machine_zscore(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["temp_zscore_machine"] = (df["temperature_c"] - df["temp_train_mean"]) / df[
        "temp_train_std"
    ].replace(0, float("nan"))
    df["pressure_zscore_machine"] = (df["pressure_bar"] - df["pressure_train_mean"]) / df[
        "pressure_train_std"
    ].replace(0, float("nan"))
    return df.drop(
        columns=["temp_train_mean", "temp_train_std", "pressure_train_mean", "pressure_train_std"]
    )
...
def compute_incident_lookback_features(df: pd.DataFrame, df_inc: pd.DataFrame) -> pd.DataFrame:
    ...
    for machine in df["machine_id"].unique():
        ...
        for idx, h in hours.items():
            w24 = evts[
                (evts["occurred_at"] >= h - pd.Timedelta(hours=24)) & (evts["occurred_at"] < h)
            ]
            w7d = evts[
                (evts["occurred_at"] >= h - pd.Timedelta(days=7)) & (evts["occurred_at"] < h)
            ]
            past = occ[occ < np.datetime64(h)]
            df.at[idx, "incident_count_prev_24h"] = len(w24)
            df.at[idx, "incident_max_severity_prev_24h"] = (
                int(w24["severity"].max()) if len(w24) else 0
            )
            df.at[idx, "incident_count_prev_7d"] = len(w7d)
            df.at[idx, "hours_since_last_incident"] = (
                (np.datetime64(h) - past[-1]) / np.timedelta64(1, "h")
                if len(past)
                else float("nan")
            )
            for t in TYPE_COLS:
...
def compute_maintenance_lookback_features(df: pd.DataFrame, df_maint: pd.DataFrame) -> pd.DataFrame:
    ...
    for machine in df["machine_id"].unique():
        ...
        for idx, h in hours.items():
            w30d = evts[
                (evts["performed_at"] >= h - pd.Timedelta(days=30)) & (evts["performed_at"] < h)
            ]
            past = occ[occ < np.datetime64(h)]
            df.at[idx, "maintenance_count_prev_30d"] = len(w30d)
            df.at[idx, "days_since_last_maintenance"] = (
                (np.datetime64(h) - past[-1]) / np.timedelta64(1, "D")
                if len(past)
                else float("nan")
            )
